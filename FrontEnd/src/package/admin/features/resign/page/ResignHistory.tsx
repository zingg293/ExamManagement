...
import { useGetListResignByHistoryQuery } from "@API/services/ResignApis.service";
...
function _ResignHistory() {
  const [pagination, setPagination] = useState({
    pageNumber: 1,
    pageSize: 10
  });
  const { data: ListResign, isLoading: LoadingListResign } = useGetListResignByHistoryQuery({
  ...
  //#endregion
  return (
    <div className={"ResignHistory"}>
      {contextHolder}
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Lịch sử danh sách đăng ký xin thôi việc</Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
...
export const ResignHistory = WithErrorBoundaryCustom(_ResignHistory);
