...
import { useDeleteOnLeaveMutation, useGetListOnLeaveByRoleQuery } from "@API/services/OnLeaveApis.service";
...
import { NewAndUpdateOnLeave, ViewDetailOnLeave } from "@admin/features/onLeave";
...
function _OnLeave() {
  ...
  const {
    data: ListOnLeave,
    isLoading: LoadingListOnLeave,
    refetch
  } = useGetListOnLeaveByRoleQuery({
    pageSize: pagination.pageSize,
    pageNumber: pagination.pageNumber
  });
  const templateId = ListOnLeave?.listPayload?.at(0)?.workflowInstances?.at(0)?.templateId;

  const [deleteListOnLeave, { isLoading: isLoadingDeleteListOnLeave }] = useDeleteOnLeaveMutation();
  const [modal, contextHolder] = Modal.useModal();
  const [isOpenModal, setIsOpenModal] = useState<boolean>(false);
  const [id, setId] = useState<string | undefined>(undefined);
  const handleDeleteOnLeave = async (id: string) => {
    modal.confirm({
      ...
      cancelText: "Hủy",
      okButtonProps: {
        loading: isLoadingDeleteListOnLeave
      },
      onOk: async () => {
        try {
          await deleteListOnLeave({
            listId: [id]
          }).unwrap();
          Modal.destroyAll();
        } catch (e: any) {
  ...
  const menuAction = (record: OnLeaveDTO) => {
    return (
      <Menu hidden={record?.workflowInstances?.at(0)?.isTerminated || record?.workflowInstances?.at(0)?.isCompleted}>
        ...
        <Menu.Item
          ...
          icon={<DeleteOutlined />}
          onClick={async () => {
            await handleDeleteOnLeave(record.id);
          }}
        >
  ...
  return (
    <div className={"OnLeave"}>
      ...
      <ModalContent
        ...
        title={id ? "Chỉnh sửa đăng ký ngày nghỉ" : "Thêm mới đăng ký ngày nghỉ"}
        width={"900px"}
      >
        <NewAndUpdateOnLeave id={id} AfterSave={() => setIsOpenModal(false)} />
      </ModalContent>
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Danh sách đăng ký ngày nghỉ </Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
...
export const OnLeave = WithErrorBoundaryCustom(_OnLeave);
