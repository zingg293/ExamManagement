...
import { useGetListBusinessTripByHistoryQuery } from "@API/services/BusinessTripApis.service";
...
function _BusinessTripHistory() {
  const [pagination, setPagination] = useState({
    pageNumber: 1,
    pageSize: 10
  });
  const { data: ListBusinessTrip, isLoading: LoadingListBusinessTrip } = useGetListBusinessTripByHistoryQuery({
  ...
  //#endregion
  return (
    <div className={"BusinessTripHistory"}>
      {contextHolder}
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Lịch sử danh sách đăng ký công tác</Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
...
export const BusinessTripHistory = WithErrorBoundaryCustom(_BusinessTripHistory);
