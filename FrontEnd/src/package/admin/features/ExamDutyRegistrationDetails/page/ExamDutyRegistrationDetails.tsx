...
import { NewAndUpdateExamShift } from "@admin/features/ExamShift";
import { ExamShiftDTO } from "@models/ExamShiftDTO";
import { useDeleteExamShiftMutation } from "@API/services/ExamShift.service";
import { useGetListExamShiftQuery } from "@API/services/ExamShift.service";
...
function _ExamDutyRegistrationDetails() {
  const { data: ListExamShift, isLoading: LoadingListExamShift } = useGetListExamShiftQuery({
    pageSize: 0,
    pageNumber: 0
  });
  const [deleteListExamShift, { isLoading: isLoadingDeleteListExamShift }] = useDeleteExamShiftMutation();
  ...
  const menuAction = (record: ExamShiftDTO) => {
    return (
      <Menu>
        <Menu.Item
          key="1"
  ...
  const columns: ColumnsType<ExamShiftDTO> = [
    {
      title: "Tên",
      dataIndex: "ExamShiftName",
      key: "ExamShiftName"
    },
    {
      title: "Trạng thái",
  ...
  const propsTable: TableProps<ExamShiftDTO> = {
    scroll: {
      x: 800
    },
    bordered: true,
    ...
    dataSource: ListExamShift?.listPayload,
    // onChange: handleChange,
    loading: LoadingListExamShift,
    pagination: {
      total: ListExamShift?.totalElement,
      pageSize: 10,
  ...
  return (
    <div className={"unit"}>
      <ModalContent
        ...
        width={"600px"}
      >
        <NewAndUpdateExamShift setVisible={setIsOpenModal} id={id} />
      </ModalContent>
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          ...
          >
            ...
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  const result = await deleteListExamShift({
                    idExamShift: selectedRowKeys as string[]
                  }).unwrap();
            ...
            >
              <Button
                danger
                type="primary"
                loading={isLoadingDeleteListExamShift}
                disabled={!(selectedRowKeys.length > 0)}
                icon={<DeleteOutlined />}
              >
...
export const ExamDutyRegistrationDetails = WithErrorBoundaryCustom(_ExamDutyRegistrationDetails);
