...
import { useGetListPromotionTransferByHistoryQuery } from "@API/services/PromotionTransferApis.service";
import { WorkFlowHistory } from "@admin/components";

function _PromotionTransferHistory() {
  const [modal, contextHolder] = Modal.useModal();
  const [pagination, setPagination] = useState({
    pageNumber: 1,
    pageSize: 10
  });
  const { data: ListPromotionTransferHistory, isLoading: LoadingListPromotionTransferHistory } =
  ...
  const columns: ColumnsType<PromotionTransferDTO> = [
    ...
    },
    {
      ...
      key: "createdDate",
      filters: ListPromotionTransferHistory?.listPayload?.map((item) => ({
        ...
        <Button
          ...
          onClick={() =>
            modal.info({
              ...
              width: 1100,
              content: <DetailPromotionTransfer idPromotionTransfer={record.id} />,
              footer: null,
              closable: true,
              style: { top: 20 }
            })
  ...
  const propsTable: TableProps<PromotionTransferDTO> = {
    ...
    dataSource: ListPromotionTransferHistory?.listPayload,
    onChange: handleChange,
    loading: LoadingListPromotionTransferHistory,
    pagination: {
      total: ListPromotionTransferHistory?.totalElement,
      pageSize: 10,
  ...
  //#endregion
  return (
    <div className="PromotionTransferHistory">
      {contextHolder}
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Typography.Title level={2}> Lịch sử danh sách bổ nhiệm và điều chuyển </Typography.Title>
          <Divider />
          <Space
            style={{
              marginBottom: 16
...
export const PromotionTransferHistory = WithErrorBoundaryCustom(_PromotionTransferHistory);
