...
function blogCategoryChild() {
  const router = useRouter();
  const id = router?.query?.slug?.at(1);
  const title = router?.query?.slug?.at(0);
  ...
  useEffect(() => {
    (async () => {
      if (typeof id === "string") {
        const res = await CategoryNewsApis.getListCategoryNewsByIdParent(id);
  ...
  const SeoConfig = {
    ...
    canonical: `${globalVariable.domain}/blog-category-child`,
    openGraph: {
      ...SeoDefault.openGraph,
      title: title,
      description: "Tổng hợp tin tức tuyển dụng mới nhất",
      url: `${globalVariable.domain}/blog-category-child`
    }
  };
  return (
    <div className="Blog">
      ...
      <PageTitle
        title={title || "Tin tức tuyển dụng"}
        subTitle={title || "Tin tức tuyển dụng"}
        breadcrumb={"Home"}
        breadcrumbLink="/"
      />
