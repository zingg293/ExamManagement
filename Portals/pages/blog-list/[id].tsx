...
function BlogList() {
  ...
  useEffect(() => {
    (async () => {
      if (typeof id === "string") {
        const [apis, apisCategoryNew] = await Promise.all([
          NewsApis.getListNewsByIdCategoryNews(id, {
            pageSize: pageSize as number,
            pageNumber: 1
          }),
          CategoryNewsApis.getCategoryNewsById(id)
        ]);
        setApis(apis);
  ...
  const SeoConfig = {
    ...SeoDefault,
    title: apisCategoryNew?.payload?.nameCategory,
    description: "Tổng hợp tin tức tuyển dụng mới nhất",
    canonical: `${globalVariable.domain}/blog-category/${router.query.id}`,
    openGraph: {
      ...SeoDefault.openGraph,
      title: apisCategoryNew?.payload?.nameCategory,
      description: "Tổng hợp tin tức tuyển dụng mới nhất",
      url: `${globalVariable.domain}/blog-category/${router.query.id}`
    }
  };
  return (
    <div className="BlogCategory">
      ...
      <PageTitle
        title={apisCategoryNew?.payload?.nameCategory || "Tin tức tuyển dụng"}
        subTitle={apisCategoryNew?.payload?.nameCategory || "Tin tức tuyển dụng"}
        breadcrumb={"Home"}
        breadcrumbLink="/"
      />
